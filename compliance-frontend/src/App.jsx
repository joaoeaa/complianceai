  ...
  return (
    <>
      ...
      <div style={{ width: isMobile ? 270 : w, minHeight: "100vh", background: "#0c0f1a", display: "flex", flexDirection: "column", position: "fixed", left: 0, top: 0, zIndex: 200, overflow: "hidden", ...(isMobile ? { transform: mobileOpen ? "translateX(0)" : "translateX(-100%)", transition: "transform 0.3s ease" } : { transition: "width 0.3s ease" }) }}>
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", background: "radial-gradient(360px 260px at 0% 0%, rgba(99,102,241,0.16), transparent 70%)" }} />
        ...
        <nav style={{ flex: 1, padding: "14px 10px", display: "flex", flexDirection: "column", gap: 3 }}>
          {navItems.map(item => { const a = currentPage === item.id; return (
            <button key={item.id} onClick={() => handleNav(item.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: show ? "11px 14px" : "11px", borderRadius: 9, border: "none", cursor: "pointer", justifyContent: show ? "flex-start" : "center", background: a ? "rgba(99,102,241,0.16)" : "transparent", transition: "all 0.2s", position: "relative" }}
              onMouseEnter={e => { if (!a) e.currentTarget.style.background = "rgba(255,255,255,0.05)"; }} onMouseLeave={e => { if (!a) e.currentTarget.style.background = a ? "rgba(99,102,241,0.16)" : "transparent"; }}>
              {a && <span style={{ position: "absolute", left: 0, top: 8, bottom: 8, width: 3, borderRadius: "0 3px 3px 0", background: "linear-gradient(135deg, #6366f1, #8b5cf6)" }} />}
              <item.icon size={18} color={a ? "#a5b4fc" : "rgba(255,255,255,0.4)"} />{show && <span style={{ color: a ? "white" : "rgba(255,255,255,0.5)", fontSize: 13, fontWeight: a ? 700 : 500, fontFamily: F }}>{item.label}</span>}
            </button>
          ); })}
        </nav>
  ...
  return (
    <div>
      ...
      <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12, marginBottom: 24 }}>
        {stats.map((s, i) => (
          <div key={i} style={{ background: "white", borderRadius: 14, padding: "16px", border: "1px solid #e2e8f0", boxShadow: "0 1px 2px rgba(11,18,32,0.04), 0 8px 24px rgba(11,18,32,0.05)", transition: "transform 0.18s, box-shadow 0.18s", animation: `fadeSlideUp 0.5s ease ${i*0.07}s both` }}
            onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-2px)"; e.currentTarget.style.boxShadow = "0 12px 32px rgba(99,102,241,0.12)"; }}
            onMouseLeave={e => { e.currentTarget.style.transform = "none"; e.currentTarget.style.boxShadow = "0 1px 2px rgba(11,18,32,0.04), 0 8px 24px rgba(11,18,32,0.05)"; }}>
      ...
      {!analyzing ? (
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={e => { e.preventDefault(); setDragOver(false); handleFile(e.dataTransfer.files[0]); }} onClick={() => fileInputRef.current?.click()} style={{ border: `2px dashed ${dragOver ? "#6366f1" : file ? "#10b981" : "#cbd5e1"}`, borderRadius: 18, padding: "48px 28px", textAlign: "center", cursor: "pointer", background: dragOver ? "#eef2ff" : file ? "#f0fdf4" : "#fafbfc", transition: "all 0.3s" }}>
            ...
            {file ? (<><div style={{ width: 52, height: 52, borderRadius: 14, background: "#dcfce7", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", boxShadow: "0 8px 24px rgba(22,163,74,0.25)" }}><CheckCircle size={26} color="#16a34a" /></div><div style={{ fontSize: 15, fontWeight: 700, color: "#0f172a", fontFamily: F }}>{file.name}</div><div style={{ color: "#64748b", fontSize: 12, marginTop: 3 }}>{formatFileSize(file.size)}</div><button onClick={e => { e.stopPropagation(); setFile(null); }} style={{ marginTop: 8, background: "none", border: "none", color: "#6366f1", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>Trocar arquivo</button></>) : (<><div style={{ width: 56, height: 56, borderRadius: 16, background: dragOver ? "linear-gradient(135deg, #6366f1, #8b5cf6)" : "linear-gradient(135deg, #818cf8, #a78bfa)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", transition: "all 0.3s", boxShadow: dragOver ? "0 10px 30px rgba(99,102,241,0.45)" : "0 8px 24px rgba(99,102,241,0.3)" }}><FileUp size={26} color="white" /></div><div style={{ fontSize: 15, fontWeight: 700, color: "#0f172a", fontFamily: F }}>Arraste e solte seu documento</div><div style={{ color: "#94a3b8", fontSize: 12, marginTop: 4 }}>ou clique para selecionar • PDF ou DOCX • até 10MB</div></>)}
          </div>
...
const ReportPage = ({ docId, onBack, showToast }) => {
  ...
  return (
    <div>
      ...
      {/* Os quatro indicadores do relatorio, no mesmo nivel. O score leva o
          anel; os demais seguem o padrao de card de estatistica. */}
      <div className="report-stats" style={{ display: "grid", gap: 14, marginBottom: 22 }}>
        <div style={{ background: "white", borderRadius: 14, border: "1px solid #e2e8f0", boxShadow: "0 1px 2px rgba(11,18,32,0.04), 0 8px 24px rgba(11,18,32,0.05)", padding: "14px 18px", display: "flex", alignItems: "center", gap: 13 }}>
          <div style={{ position: "relative", width: 54, height: 54, flexShrink: 0 }}>
            <svg width={54} height={54} style={{ transform: "rotate(-90deg)" }}>
              <circle cx={27} cy={27} r={22} fill="none" stroke="#edf0f6" strokeWidth={6} />
              <circle cx={27} cy={27} r={22} fill="none" stroke={riskCol} strokeWidth={6} strokeLinecap="round" strokeDasharray={138.2} strokeDashoffset={138.2 * (1 - (analysis.risk_score || 0) / 100)} style={{ transition: "stroke-dashoffset 1s ease" }} />
            </svg>
            <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, fontWeight: 800, color: "#0f172a", fontFamily: F }}>{analysis.risk_score}</div>
          </div>
          <div>
            <div style={{ fontSize: 10, fontWeight: 600, color: "#94a3b8", textTransform: "uppercase", letterSpacing: "0.05em", fontFamily: F }}>Score de Risco</div>
            <div style={{ fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.04em", color: riskCol, fontFamily: F }}>{analysis.risk_score >= 70 ? "Alto" : analysis.risk_score >= 40 ? "Moderado" : "Baixo"}</div>
          </div>
        </div>
        {[
          ...
          <div key={i} style={{ background: "white", borderRadius: 14, border: "1px solid #e2e8f0", boxShadow: "0 1px 2px rgba(11,18,32,0.04), 0 8px 24px rgba(11,18,32,0.05)", padding: "14px 18px", display: "flex", alignItems: "center", gap: 14 }}>
    ...
    <div>
      ...
      {showF && (
        <div style={{ display: "flex", gap: 14, marginBottom: 16, padding: "14px 18px", background: "white", borderRadius: 11, border: "1px solid #e2e8f0", flexWrap: "wrap", animation: "fadeSlideUp 0.2s ease" }}>
          <div><div style={{ fontSize: 10, fontWeight: 600, color: "#64748b", marginBottom: 5, textTransform: "uppercase", letterSpacing: "0.05em", fontFamily: F }}>Status</div><div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>{[{ k: "all", l: "Todos" },{ k: "analyzed", l: "Analisados" },{ k: "processing", l: "Processando" },{ k: "error", l: "Erro" }].map(f => <button key={f.k} onClick={() => setSf(f.k)} style={{ padding: "7px 14px", borderRadius: 999, border: "1px solid", borderColor: sf === f.k ? "#c9d1fb" : "#e2e8f0", background: sf === f.k ? "#eef2ff" : "white", color: sf === f.k ? "#4f46e5" : "#64748b", fontSize: 11.5, fontWeight: 600, cursor: "pointer", fontFamily: F, transition: "all 0.15s" }}>{f.l}</button>)}</div></div>
          {clients.length > 0 && (
            <div>
      ...
      <div style={{ background: "white", borderRadius: 14, border: "1px solid #e2e8f0", overflow: "hidden" }}>
        ...
        {loading ? <div style={{ padding: 36, textAlign: "center" }}><Loader2 size={20} color="#6366f1" style={{ animation: "spin 1s linear infinite" }} /></div> : documents.length === 0 ? <div style={{ padding: 36, textAlign: "center", color: "#94a3b8", fontSize: 13 }}>Nenhum documento encontrado.</div> : documents.map((doc, i) => { const st = getStatusStyle(doc.status); return (
          <div key={doc.id} className="history-row" style={{ display: "grid", padding: "12px 18px", borderBottom: i < documents.length - 1 ? "1px solid #f8fafc" : "none", alignItems: "center", transition: "background 0.15s" }} onMouseEnter={e => e.currentTarget.style.background = "#fafbfc"} onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
            ...
            <div>{doc.risk_score != null ? <div style={{ display: "flex", alignItems: "center", gap: 5 }}><div style={{ width: 44, height: 5, background: "#edf0f6", borderRadius: 3, overflow: "hidden" }}><div style={{ width: `${doc.risk_score}%`, height: "100%", background: getRiskColor(doc.risk_score), borderRadius: 3 }} /></div><span style={{ fontSize: 12.5, fontWeight: 800, color: getRiskColor(doc.risk_score), fontFamily: F, fontVariantNumeric: "tabular-nums" }}>{doc.risk_score}</span></div> : <span style={{ color: "#cbd5e1", fontSize: 11 }}>sem score</span>}</div>
            <div><span style={{ fontSize: 10, fontWeight: 700, color: st.color, background: st.bg, padding: "3px 10px", borderRadius: 20, fontFamily: F }}>{st.label}</span></div>
