...
@router.get("/feedback/learnings", response_model=list[dict])
async def get_learning_data(
    limit: int = Query(50, ge=1, le=200),
    db: AsyncSession = Depends(get_db),
    current_user=Depends(get_current_user),
):
    """Get aggregated learning data from alert feedback — used internally by AI analysis.
    Returns: list of {rule_name, total, correct, incorrect, false_positive_rate, sample_comments}"""
    from app import AlertFeedback
    ...
    result = await db.execute(
        ...
        .order_by(func.count(AlertFeedback.id).desc())
        .limit(limit)
    )
